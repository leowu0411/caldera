...
def add_write_tag_subparser(subparsers):
    parser = subparsers.add_parser(
        "write_tag",
        help="Write Tag: Write a value to a specific tag in the device",
    )
    parser.add_argument(
        "tag_id",
        type=int,
        help="The tag ID to write to",
    )
    parser.add_argument(
        "value",
        type=float,
        help="The value to be written to the tag",
    )
    add_device_id_arg(parser)

def add_read_tag_parser(subparsers):
    parser = subparsers.add_parser(
        "read_tag",
        help="Read Tag: Read a specific tag in the device",
    )
    parser.add_argument(
        "tag_id",
        type=int,
        help="The tag ID to read",
    )
    add_device_id_arg(parser)
...
def create_arg_parser():
    ...
    add_write_tag_subparser(subparsers)
    add_read_tag_parser(subparsers)

    return parser


def do_action(client, args, helper):
    ...
    elif args.action.lower() == "write_tag":
        print("[*] Write tag")
        offset = helper.get_registers(args.tag_id)
        words  = helper.encode(args.value)
        result = client.write_registers(offset, words, args.device_id)
        if result.isError():
            print("Write failed")
            log.error("Write error")
        else:
    ...
    elif args.action.lower() == "read_tag":
        try:
            offset = helper.get_registers(args.tag_id)
            rs = client.read_holding_registers(offset, helper._word_num, args.device_id)
            real = helper.decode(rs.registers)
            print(f"TAG {args.tag_id} = {real}")
        except Exception as err:
            print(f"Read failed: {err}")
            log.error(err)
